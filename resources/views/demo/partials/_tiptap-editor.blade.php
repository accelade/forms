{{-- TipTap Editor Component Section --}}
@props(['prefix' => 'a'])

<section class="rounded-xl p-6 mb-6 border border-[var(--docs-border)]" style="background: var(--docs-bg-alt);">
    <div class="flex items-center gap-3 mb-2">
        <span class="w-2.5 h-2.5 bg-purple-500 rounded-full"></span>
        <h3 class="text-lg font-semibold" style="color: var(--docs-text);">TipTap Editor</h3>
    </div>
    <p class="text-sm mb-6" style="color: var(--docs-text-muted);">
        A powerful WYSIWYG rich text editor powered by TipTap with full extension support. Compatible with Filament TipTap Editor API.
    </p>

    <div class="space-y-6 mb-6">
        {{-- Default Profile --}}
        <div class="rounded-xl p-4 border border-purple-500/30" style="background: rgba(139, 92, 246, 0.1);">
            <h4 class="font-medium mb-3 flex items-center gap-2" style="color: var(--docs-text);">
                <span class="text-xs px-2 py-1 bg-purple-500/20 text-purple-500 rounded">Default</span>
                Full-Featured Editor
            </h4>
            <x-forms::tiptap-editor
                name="tiptap_content"
                profile="default"
                placeholder="Start writing your content..."
            />
        </div>

        {{-- Simple Profile --}}
        <div class="rounded-xl p-4 border border-emerald-500/30" style="background: rgba(16, 185, 129, 0.1);">
            <h4 class="font-medium mb-3 flex items-center gap-2" style="color: var(--docs-text);">
                <span class="text-xs px-2 py-1 bg-emerald-500/20 text-emerald-500 rounded">Simple</span>
                Minimal Toolbar
            </h4>
            <x-forms::tiptap-editor
                name="tiptap_simple"
                profile="simple"
                placeholder="Essential tools only..."
            />
        </div>

        {{-- With Character Count --}}
        <div class="rounded-xl p-4 border border-amber-500/30" style="background: rgba(245, 158, 11, 0.1);">
            <h4 class="font-medium mb-3 flex items-center gap-2" style="color: var(--docs-text);">
                <span class="text-xs px-2 py-1 bg-amber-500/20 text-amber-500 rounded">Limited</span>
                With Character Count
            </h4>
            <x-forms::tiptap-editor
                name="tiptap_limited"
                profile="simple"
                :max-length="500"
                placeholder="Maximum 500 characters..."
            />
        </div>

        {{-- RTL Support --}}
        <div class="rounded-xl p-4 border border-cyan-500/30" style="background: rgba(6, 182, 212, 0.1);">
            <h4 class="font-medium mb-3 flex items-center gap-2" style="color: var(--docs-text);">
                <span class="text-xs px-2 py-1 bg-cyan-500/20 text-cyan-500 rounded">RTL</span>
                Right-to-Left Support
            </h4>
            <x-forms::tiptap-editor
                name="tiptap_rtl"
                label="Arabic Content"
                profile="simple"
                direction="rtl"
                placeholder="اكتب هنا..."
            />
        </div>
    </div>

    {{-- Code Examples --}}
    <div class="space-y-4">
        <h4 class="font-medium" style="color: var(--docs-text);">Usage Examples</h4>

        <x-accelade::code-block language="php" title="TipTap Editor Examples">
use Accelade\Forms\Components\TipTapEditor;

// Full-featured with default profile
TipTapEditor::make('content')
    ->profile('default')
    ->placeholder('Start writing...');

// Simple profile
TipTapEditor::make('comment')
    ->profile('simple');

// Custom tools with separators
TipTapEditor::make('body')
    ->tools([
        'bold', 'italic', 'underline',
        '|',
        'h2', 'h3',
        '|',
        'bulletList', 'orderedList',
        '|',
        'link',
    ]);

// With character limit
TipTapEditor::make('summary')
    ->maxLength(500)
    ->showCharacterCount();

// RTL support
TipTapEditor::make('content')
    ->rtl();

// Output formats
TipTapEditor::make('content')
    ->output('html');  // or 'json', 'text'
        </x-accelade::code-block>
    </div>

    {{-- Available Tools Reference --}}
    <div class="mt-6 pt-6 border-t border-[var(--docs-border)]">
        <h4 class="font-medium mb-4" style="color: var(--docs-text);">Available Tools</h4>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">Text Formatting</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['bold', 'italic', 'underline', 'strike', 'highlight', 'code'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>

            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">Headings</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'paragraph'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>

            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">Lists & Blocks</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['bulletList', 'orderedList', 'blockquote', 'codeBlock'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>

            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">Alignment</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['alignLeft', 'alignCenter', 'alignRight', 'alignJustify'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>

            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">Media</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['link', 'unlink', 'image', 'table', 'horizontalRule'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>

            <div class="rounded-lg p-3" style="background: var(--docs-bg);">
                <h5 class="text-sm font-medium mb-2" style="color: var(--docs-text);">History</h5>
                <div class="flex flex-wrap gap-1">
                    @foreach(['undo', 'redo', 'clearFormatting'] as $tool)
                        <span class="text-xs px-2 py-1 rounded font-mono" style="background: var(--docs-bg-alt); color: var(--docs-text-muted);">{{ $tool }}</span>
                    @endforeach
                </div>
            </div>
        </div>
    </div>
</section>
