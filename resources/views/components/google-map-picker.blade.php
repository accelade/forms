...
@php
    ...
    $apiKey = $field->getApiKey();
    $height = $field->getHeight();

    $config = $field->getPickerConfig();

    $containerClasses = 'google-map-picker-wrapper relative rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden';
...
<div class="{{ config('forms.styles.field', 'mb-4') }}">
    ...
    <div
        ...
        data-map-picker-config="{{ json_encode($config) }}"
        data-map-api-key="{{ $apiKey }}"
        @if($isDisabled) data-disabled="true" @endif
    >
        {{-- Map Container --}}
        <div class="map-picker-container" style="height: {{ $height }};"></div>

        {{-- Loading State --}}
        <div class="map-picker-loading absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-800">
        ...
        <div class="absolute left-3 top-3 z-10 w-64">
            <div class="relative">
                <input
                    type="text"
                    class="map-picker-search w-full rounded-lg border-0 bg-white py-2 pl-9 pr-3 text-sm shadow-md placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
        ...
        <button
            type="button"
            class="map-picker-locate absolute bottom-3 right-3 z-10 rounded-lg bg-white p-2 shadow-md transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:hover:bg-gray-600"
                ...
                <div class="flex items-start gap-2">
                    ...
                    <div>
                        <p class="text-sm text-gray-600 dark:text-gray-300">
                            Set <code class="rounded bg-gray-100 px-1 text-xs dark:bg-gray-700">GOOGLE_MAPS_API_KEY</code> in .env
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-2 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
        <span>Lat: <span class="map-picker-lat-display font-mono">{{ number_format($currentLat, 6) }}</span></span>
        <span>Lng: <span class="map-picker-lng-display font-mono">{{ number_format($currentLng, 6) }}</span></span>
    </div>
...
<style>
    .google-map-picker-wrapper[data-initialized="true"] .map-picker-loading {
        display: none;
    }
</style>
